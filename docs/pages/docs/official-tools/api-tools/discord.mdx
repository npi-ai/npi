...
<Steps>
    ...
    Follow this guide to create a Discord chatbot and grab your access token: https://discordpy.readthedocs.io/en/stable/discord.html
...
1. Send a Greeting Message to a User
    ...
    <Tabs items={['CLI', 'Python']}>
        ...
        <Tabs.Tab title='Python'>
            ```python
            discord.chat("Send a greeting message to XXX (user id: {{user_id}}).")
            ```
        </Tabs.Tab>
    </Tabs>
...
2. Ask for User Confirmation and Await Their Reply
    ...
    <Tabs items={['CLI', 'Python']}>
        ...
        <Tabs.Tab title='Python'>
            ```python
            discord.chat("Send a direct message to XXX (user id: {{user_id}}) asking them to confirm next Monday's meeting, and wait for their reply.")
            ```
        </Tabs.Tab>
    </Tabs>
